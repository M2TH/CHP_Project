...
vector<double> F_b(double Lx,double Ly, double beta, double gamma, double dt, double t, double D, int cas, int Nx, int Ny, vector<double> U)
{
    ...
    F[0]=U[0]+dt*f(x,y,t,Lx,Ly,cas)-D*beta*h(0,y,Lx,Ly,cas)-D*gamma*g(x,0,Lx,Ly,cas); // cas j=0 (j boucle dans les blocs)
    
    for(int j=1;j<Nx-1;j++) // cas pour j différent de 0 et Nx-1
    {
        x=x+dx;
        F[j]=U[j]+dt*f(x,y,t,Lx,Ly,cas)-D*gamma*g(x,0,Lx,Ly,cas);
    }
    ...
    F[Nx-1]=U[Nx-1]+dt*f(x,y,t,Lx,Ly,cas)-D*beta*h(x+dx,y,Lx,Ly,cas)-D*gamma*g(x,0,Lx,Ly,cas);
    ...
    for (int i=1;i<Ny-1;i++) 
    {
        ...
        F[df]=U[df]+dt*f(x,y,t,Lx,Ly,cas)-D*beta*h(0,y,Lx,Ly,cas);
        ...
        // cas j=Nx-1
        x=x+dx;
        F[df+Nx-1]=U[df+Nx-1]+dt*f(x,y,t,Lx,Ly,cas)-D*beta*h(x+dx,y,Lx,Ly,cas);
    }
    ...
    F[df]=U[df]+dt*f(x,y,t,Lx,Ly,cas)-D*beta*h(0,y,Lx,Ly,cas)-D*gamma*g(x,y+dy,Lx,Ly,cas);
    ...
    // cas j=Nx-1
    x=x+dx;
    F[df+Nx-1]=U[df+Nx-1]+dt*f(x,y,t,Lx,Ly,cas)-D*beta*h(x+dx,y,Lx,Ly,cas)-D*gamma*g(x,y+dy,Lx,Ly,cas);
